  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formRef.current) {
        ...
        toast.success(
          "Proposal brief sent successfully! We will get back to you shortly."
        );
        onClose();
      }
    } catch (err) {
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:outline-none focus:border-slate-900"
            >
              <option value="Custom Next.js Web App">
                Custom Next.js Web App
              </option>
              <option value="AgriTech / Multi-Catalog Portal">
                AgriTech / Multi-Catalog Portal
              </option>
              <option value="Bespoke Architecture Showcase">
                Bespoke Architecture Showcase
              </option>
            </select>
          </div>
          ...
          >
            <span>
              {loading ? "Sending Brief..." : "Submit Proposal Brief"}
            </span>
